...
# MAGIC %sql
# MAGIC --drop view saetram.v_ref_topo_chainage_tram
# MAGIC --select * from allColumns where upper(columnName) like '%NO_PERIODE_TOPO%' and database <>'default' order by database, tableName
# MAGIC --select * from allColumns where upper(columnName) like '%SK_%' and database <>'default' order by database, tableName
# MAGIC 
# MAGIC select database, tableName, 
# MAGIC concat('ALTER TABLE ',database,'.',tableName,' SET TBLPROPERTIES (delta.autoOptimize.optimizeWrite = true, delta.autoOptimize.autoCompact = true)')
# MAGIC from allColumns
# MAGIC group by database, tableName
