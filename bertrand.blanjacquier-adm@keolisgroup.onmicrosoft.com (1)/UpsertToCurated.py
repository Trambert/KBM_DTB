...
def createOrUpdateDelta(newDf, save_path, key, object):
    ...
    if isDelta:
        #Table Delta exists, doing merge
        deltaTable = DeltaTable.forPath(spark, save_path)
        deltaTable
            .alias("old") #rename existing Table as old
            .merge(source = newDf.alias("updates"), #rename incoming dataset as new
                condition = expr("old."+key+" = updates."+key) #definition of deduplication keys
                  )
            .whenMatchedUpdateAll() #all column updates if key reconciliation
            .whenNotMatchedInsertAll() #insertion if no key reconciliation
            .execute()
    else:
        #Table Delta not existing
        table_name = object
        ...
        #delta table creation in metastore
